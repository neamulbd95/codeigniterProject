...
<body>
	...
	{cart}
		...
		<tr align="left">
                    ...
                    <td>{Item_Catagory}</td>
                    <td>{Item_Brand}</td>
                    <td><img src="{Item_Picture}" alt="" style="width:100px; height:100px;"></a></td>
                    <td>{Item_Price}</td>
                    <td>{Quantity}</td>
                    <td>{total_price}</td>   
                    <td style="background-color: red"><a href="http://localhost/atp3project/cart/deleteCartItem/{Item_ID}">Delete</a></td>
		
		</tr>
     {/cart}       
                <tr>
                    <td colspan="7">Total Cost</td>
                    <td>{final_price}{Total_PRICE}{/final_price}</td>
                </tr>
	</table>

        <form  method="post" action="/atp3project/cart/confirmation">
            <input type="submit" class="button1" id ="ProfileBu" name="buttonSubmit" value="Buy">
        </form>
